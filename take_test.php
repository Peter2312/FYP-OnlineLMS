...
<?php $query1 = mysqli_query($connection, "SELECT * from student_class_quiz where student_id = '$session_id' and class_quiz_id = '$class_quiz_id' ")or die(mysqli_error($connection));
...
if ($count > 0){
 ...
 mysqli_query($connection, "INSERT INTO student_class_quiz (class_quiz_id,student_id,student_quiz_time) values('$class_quiz_id','$session_id','$quiz_time')");
}
 ?>
                ...
                <div class="span9" id="content">
                     <div class="row-fluid">
                        ...
                        <div id="block_bg" class="block">
							...
							<?php
if($_GET['test'] == 'ok'){
/* $sqlp = mysqli_query($connection, "SELECT * FROM groupcode WHERE course_code = '".$row['course_code']."'"); */
$sqlp = mysqli_query($connection, "SELECT * FROM class_quiz WHERE class_quiz_id = '$class_quiz_id'")or die(mysqli_error($connection));
$rowp = mysqli_fetch_array($sqlp);
/* mysqli_query($connection, "UPDATE students SET `time-left` = ".$rowp['time']." WHERE stud_id = '".$_SESSION['user_id']."'"); */
/* echo $rowp['time']; */
$x=0;
?>
										<script>
...
<?php
										$sqla = mysqli_query($connection, "SELECT * FROM class_quiz 
										LEFT JOIN quiz ON quiz.quiz_id  = class_quiz.quiz_id
										where lecturer_class_id = '$get_id' 
										order by date_added DESC ")or die(mysqli_error($connection));
										/* $row = mysqli_fetch_array($sqla); */
										$rowa = mysqli_fetch_array($sqla);
					
										/* $rowa   = $row['quiz_id']; */
/* $sqla = mysqli_query($connection, "SELECT * FROM class_quiz WHERE course_code = '".$row['course_code']."'"); */
...
<?php
	$sqlw = mysqli_query($connection, "SELECT * FROM quiz_question where quiz_id = '$quiz_id'  ORDER BY RAND()");
...
<?php
	...
	$sqly = mysqli_query($connection, "SELECT * FROM answer WHERE quiz_question_id = '".$roww['quiz_question_id']."'");
...
<?php
	...
	for($x=1;$x<=$x1;$x++){
		...
		$sql = mysqli_query($connection, "SELECT * FROM quiz_question WHERE quiz_question_id = ".$x2."");
	...
	mysqli_query($connection, "UPDATE student_class_quiz SET `student_quiz_time` = 3600, `grade` = '".$score." out of ".($x-1)."' WHERE student_id = '$session_id' and class_quiz_id = '$class_quiz_id'")or die(mysqli_error($connection));
?>
<script>
...
<br />
<?php
/* $sql = mysqli_query($connection, "SELECT * FROM students WHERE stud_id = '".$_SESSION['user_id']."'");
$row = mysqli_fetch_array($sql);
...
/* 	} else if(!is_null($row['grade'])){
		$sqlg = mysqli_query($connection, "SELECT * FROM groupcode WHERE course_code = '".$row['course_code']."'");
		$rowg = mysqli_fetch_array($sqlg);
		echo "You have already taken <b>".$rowg['course_title']."</b> - <b>".$rowg['course_code']."</b> test.";
	}
	if($row['grade']!=''){
		mysqli_query($connection, "UPDATE students SET `time-left` = 3600 WHERE stud_id = '".$_SESSION['user_id']."'");
