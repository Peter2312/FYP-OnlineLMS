                     ...
                     <div class="row-fluid">
                        ...
                        <div id="block_bg" class="block">
                            ...
                            <div class="block-content collapse in">
                                <div class="span12">
								<?php
										$query = mysqli_query($connection, "SELECT * FROM quiz_question
										LEFT JOIN question_type on quiz_question.question_type_id = question_type.question_type_id
										where quiz_id = '$get_id' and quiz_question_id = '$quiz_question_id'  order by date_added DESC ")or die(mysqli_error($connection));
							    ...
							    <form class="form-horizontal" method="post">
										...
										<div class="control-group">
											...
											<div class="controls">			
												<select id="qtype" name="question_tpye" required>
													...
													<?php
													$query_question = mysqli_query($connection, "SELECT * from question_type")or die(mysqli_error($connection));
										...
										<div id="opt11">
<?php
	$sqlz = mysqli_query($connection, "SELECT * FROM answers WHERE q_id = ".$_GET['edit']."");
...
<div id="opt12">
	...
	<input name="correctt" <?php if($row['answer'] == 'False'){ echo 'checked'; }?> value="f" type="radio">False<br /><br />
		...
		if ($type  == '2'){
				mysqli_query($connection, "INSERT INTO quiz_question (quiz_id,question_text,date_added,answer,question_type_id) 
			values('$get_id','$question',NOW(),'".$_POST['correctt']."','$type')")or die(mysqli_error($connection));
		}else{
	
		mysqli_query($connection, "INSERT INTO quiz_question (quiz_id,question_text,date_added,answer,question_type_id) 
		values('$get_id','$question',NOW(),'$answer','$type')")or die(mysqli_error($connection));
		$query = mysqli_query($connection, "SELECT * from quiz_question order by quiz_question_id DESC LIMIT 1")or die(mysqli_error($connection));
		$row = mysqli_fetch_array($query);
		$quiz_question_id = $row['quiz_question_id'];
		
		mysqli_query($connection, "INSERT INTO answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans1','A')")or die(mysqli_error($connection));
		mysqli_query($connection, "INSERT INTO answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans2','B')")or die(mysqli_error($connection));
		mysqli_query($connection, "INSERT INTO answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans3','C')")or die(mysqli_error($connection));
		mysqli_query($connection, "INSERT INTO answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans4','D')")or die(mysqli_error($connection));
